...
describe('SyncService', () => {
	...
	describe('fetchAndUpdateLocal', () => {
		...
		it('should fetch profile from Supabase and update local database', async () => {
			...
			expect(mockDb.runAsync).toHaveBeenCalledWith(
				...
				[userId, 100, '2024-01-01T00:00:00Z', '2024-01-01T00:00:00Z'],
			)
	...
	describe('updateLocalProfile', () => {
		...
		it('should update local profile without syncing', async () => {
			...
			expect(mockDb.runAsync).toHaveBeenCalledWith(
				...
				expect.arrayContaining([newXp, 1, expect.any(String), userId]),
			)
		})

		it('should queue sync and call syncToSupabase when sync=true', async () => {
			const userId = 'user-123'

			// Spy on syncToSupabase so we don’t actually run it
			const syncSpy = jest.spyOn(syncService, 'syncToSupabase').mockResolvedValue(undefined)
	...
	describe('Multiple operations', () => {
		...
		it('should handle multiple consecutive fetches', async () => {
			...
			const mockSupabaseFrom = jest.fn().mockImplementation((table) => ({
				select: jest.fn().mockReturnValue({
